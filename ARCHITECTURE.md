...
**Conversation storage:** OpenClaw 2026.9.4 owns session indexes and transcripts in its agent SQLite databases. The controller's `SessionsRuntime` reads them through the gateway's paginated `sessions.list` and `chat.history` APIs; rename, reset, delete, fork, and rewind also use gateway operations. Existing `.jsonl` route IDs remain opaque identifiers, not paths to writable transcripts. File readers remain available for legacy fixtures and offline archives.
